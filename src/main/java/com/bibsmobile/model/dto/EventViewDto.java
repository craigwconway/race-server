...
public class EventViewDto {
	...
	public static String fromEventToDto(Event event ) {
		ObjectMapper mapper = new ObjectMapper();
		try {
			return mapper.writeValueAsString(new EventViewDto(event));
		} catch (JsonProcessingException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			return null;
	...
	public static String fromEventsToDtoArray(Collection<Event> events ) {
		List <EventViewDto> dtos = new ArrayList <EventViewDto>();
		for(Event event : events) {
			dtos.add(new EventViewDto(event));
		}
		ObjectMapper mapper = new ObjectMapper();
		try {
