...
public class TimerConfigController {
    ...
    private synchronized Timer getTimer(long id) {
        ...
        Timer timer = null;
        if (this.timers.containsKey(timerConfig)) {
            System.out.println("Found cached timer!");
            timer = this.timers.get(timerConfig);
        } else {
            ...
            this.timers.put(timerConfig, timer);
    ...
    public String write(@PathVariable(value = "id") long id) {
         ...
         * UserProfile user =
         * UserProfile.findUserProfilesByUsernameEquals(username
         * ).getSingleResult();
         * System.out.println("writes group "+user.getUserGroup().getName());
         * 
         * // TODO license check
         * 
         * int writes = user.getUserGroup().getBibWrites(); if(writes < 1)
         * return "none"; try{ timer.write(id); }catch(Exception e){
         * System.out.println("writes error "+e.getMessage());
         * if(e.getMessage().equals("No tags present")){ return "notags"; }else
         * if(e.getMessage().equals("Too many tags present")){ return
         * "manytags"; }else{ return "false"; } }
