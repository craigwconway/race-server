...
public class RegistrationsRestController {
	...
	public ResponseEntity<String> search(@RequestParam("event") Long eventId, @RequestParam(value="firstName", required=false) String firstName, @RequestParam(value="lastName", required=false) String lastName, @RequestParam(value="start", required=false) Integer start, @RequestParam(value="count", required=false) Integer count, HttpServletRequest request) {
		try{
			...
			for (Cart c : carts) {
				...
				for (ShortCart.ShortCartItem ci : r.getCartItems()) {
					if (firstNameGiven && lastNameGiven)
						foundUser = ci.getUser().getFirstName() != null && ci.getUser().getFirstName().equalsIgnoreCase(firstName) &&
								ci.getUser().getLastName() != null && ci.getUser().getLastName().equalsIgnoreCase(lastName);
				...
				if (foundUser) registrations.add(r);
			}
	...
	/**
	 * JSON output objects
	 * all the getters are needed by the Jackson json serializer, but the compiler doesn't know that
	 *
	 */
